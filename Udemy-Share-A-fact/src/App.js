  ...
  return (
    <>
      ...
      <main className="main">
        ...
        {isLoading ? <Loader /> : <FactsList facts=
...
function NewFactForm({ setFacts, SetShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    if (text && isValidHttpUrl(source) && category && text.length <= 200) {
      ...
      if (!error) setFacts((facts) => [data, ...facts]);
  ...
  return (
    <form className="fact-form" name="fact-form" id="fact-form" onSubmit={handleSubmit}>
    ...
    <aside>
      <ul>
        ...
        <li className="category"><button className="btn 
              btn-all-categories" onClick={() => setCurrentCategory("all")}>All</button>
        </li>
...
function FactsList({ facts, setFacts }) {
  ...
  if (facts.length === 0)
    return (
      <p className="message">
        No facts for this category yet! Create the
        first one ✌️
      </p>
    );
  ...
  return (
    <section>
      <ul className="facts-list">
        {facts.map((fact) => (
          <Fact key={fact.id} fact={fact} setFacts=
            {setFacts} />
        ))}
      </ul>
